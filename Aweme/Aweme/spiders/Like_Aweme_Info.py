...
class AwemeInfoSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for aweme_list in aweme_lists:
            ...
            item1['uid'] = re.search('user_id=(\d+)',response.meta['url']).group(1) # user与aweme关系表uid
            ...
            if music:
                try:
                    item['owner_nickname'] =re.findall('\w+',music['owner_nickname'])
                ...
                try:
                    item['author'] =re.findall('\w+',music['author'])
                ...
                item['cover_medium'] = music['cover_medium']['url_list'][0]  # 封面中图地址
                ...
                item['title'] = music['title']  # 背景音乐名称
                try:
                    item['title'] =re.findall('\w+',music['title'])
            ...
            item['download_addr'] = video['download_addr']['url_list'][0]  # 视频下载地址
            item['dynamic_cover'] = video['dynamic_cover']['url_list'][0]  # 动态封面地址
            ...
            item['max_cursor'] = data['max_cursor']
