...
"""
需求：爬取发布抖音作品列表，并实现时时更新（点赞、评论、分享数）
方式: 基于uid拼接抓取url
数据库：存入两张表，一张为发布作品详单级信息表，一张为KOL的ID与作品ID关联表
        ...
        for aweme_list in aweme_lists:
            ...
            if music:
                ...
                if len(play_url):  # 播放列表可能为空 添加判断
                    item['play_url'] = music['play_url']['url_list'][
                        0]  # 音乐播放地址
                else:
                    item['play_url'] = ''
                item['source_platform'] = music['source_platform']  # 来源平台？
                try:
                    item['title'] = re.findall('\w+', music['title'])  # 北京音乐名称
            ...
            if video:
                ...
                item['play_addr_url'] = video['play_addr']['url_list'][
                    0]  # 视频播放url
