...
"""
目的：爬取KOL个人信息
方式：根据KOL的ID拼接用户信息URL并访问、存储
数据库：MySQL表userinfo表、userinfo_concern表
...
class FansSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        """
        初始化爬取URL； 从预先准备的表中获取目标KOLID，拼接抓取URL
        :param args:
        :param kwargs:
        """
    ...
    def start_requests(self):
        """
        根据KOL的ID拼接抓取URL，并重新获取加密参数
        :return:
        """
        for user_id in self.User_ID_List:
            initial_url = self.url.format(user_id[0])
            start_url = Encrypt.get_aweme_token(initial_url)
    ...
    def parse(self, response):
        try:
            ...
            item["aweme_fans_count"] = followers_detail[0][
                'fans_count']  # 抖音粉丝数
            item["aweme_apple_id"] = followers_detail[0][
                'apple_id']  # 抖音apple_id
            item["toutiao_fans_count"] = followers_detail[1][
                'fans_count']  # 头条粉丝数
            item["toutiao_apple_id"] = followers_detail[1][
                'apple_id']  # 头条apple_id
            ...
            item["custom_verify"] = re.findall(r'\w+',user_info['custom_verify'])  #
            ...
            item["is_gov_media_vip"] = user_info[
                'is_gov_media_vip']  # 是否是政府单位媒体
            ...
            # 企业认证原因
            item["enterprise_verify_reason"] = user_info[
                'enterprise_verify_reason']
            item["commerce_user_level"] = user_info[
                'commerce_user_level']  # 商务等级
            ...
            item1["aweme_fans_count"] = followers_detail[0]['fans_count']
            item1['following_count'] = user_info['following_count']
            item1["aweme_count"] = user_info['aweme_count']
            item1["favoriting_count"] = user_info['favoriting_count']

            yield item
            yield item1
        except Exception as error:
            log.error(error)
            print("用户ID：" + str(re.search('user_id=(\d+)', response.url).group(1)))
